...
如果你跳过了种子的校验（**不推荐，萌新请不要这样做**），导致99%状态的种子开始，进入了下载状态（选错种子文件位置同理），这时候请不要惊慌，先等待这次汇报完毕后的1分钟后再暂停种子，等待汇报完毕，走上面的不完整流程到100%时，再次进入做种状态即可正常做种，千万不要手快直接删除了种子，否则你将可能需要等待1到2小时，Tracker服务器超时消除你的peer记录的时候才能再次做种
